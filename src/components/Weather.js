...
const Weather = () => {
  ...
  useEffect(() => {
    ...
    const fetchHourlyForecast = async () => {
      try {
        ...
        // Filter forecast data to get hours that are greater than the current time
        const upcomingHours = data.list.filter(hour => hour.dt > currentTime);
  
        // Get the next 6 hours
        setHourlyForecast(upcomingHours.slice(0, 6));
      } catch (error) {
  ...
  const getSongRecommendations = (description, isDaytime) => {
    const songs = {
      clear: {
        day: [
          { trackId: '7AH2SbY4kpTmvxp9zAsHDS', trackName: 'Ennavale Adi Ennavale - Kadhalan' },
          { trackId: '1BHP8Y2A8k4NOEWj9viT1i', trackName: 'Oru Deivam Thantha Poove - Kannathil Muthamittal' },
          { trackId: '0cVrLxeQjiAcftuGzp32qY', trackName: 'New York Nagaram - Sillunu Oru Kaadhal' },
          { trackId: '2E3pO72BvxtdE7FbvvJG8r', trackName: 'Anjali Anjali - Duet' },
          { trackId: '0MX3B3sMZFTUuCRG3ED9lj', trackName: 'Roja Roja - Kadhalar Dhinam' }
        ],
        night: [
          { trackId: '0vHL805JOOZUyHNWmmzC2J', trackName: 'Kadhal Rojave - Roja' },
          { trackId: '6ZIGtwWzDNxgPoaWJ0vami', trackName: 'Uyire Uyire - Bombay' },
          { trackId: '7F2yXXaikB9SQNVZRjDSVo', trackName: 'Vennilave Vennilave - Minsara Kanavu' },
          { trackId: '3hp56aHySTDt9bdZiwdDO1', trackName: 'Pachai Nirame - Alaipayuthey' },
          { trackId: '5p0EV9zCL2PDbRciPXFCw1', trackName: 'Snehithane - Alaipayuthey' }
        ]
      },
      rain: {
        day: [
          { trackId: '1Ufp29R3oatZIrDsEXwzjV', trackName: 'Kannalanae - Bombay' },
      ...
      drizzle: {
        day: [
          { trackId: '1Ufp29R3oatZIrDsEXwzjV', trackName: 'Kannalanae - Bombay' },
          { trackId: '7AH2SbY4kpTmvxp9zAsHDS', trackName: 'Ennavale Adi Ennavale - Kadhalan' },
          { trackId: '', trackName: 'Oru Deivam Thantha Poove - Kannathil Muthamittal' },
          { trackId: '3WqTLAB7u84IHk91AQZXHa', trackName: 'New York Nagaram - Sillunu Oru Kaadhal' },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 rounded-2xl via-purple-900 to-pink-900 py-12 px-4 sm:px-6 lg:px-8">
          ...
          <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="bg-white/10 p-8 rounded-3xl shadow-lg text-white">
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="mt-4">
                  <h3 className="text-2xl font-bold">Weather-Updates</h3>
